...
import BitlyShorten as BitlyShorten
import BitlyExpand as BitlyExpand
...
class BitlyExpandCommand(sublime_plugin.WindowCommand):
  def run(self, edit):
    ...
    #   logger.debug(self.settings.get("api_login"))
    #   logger.debug("user " + str(self.view.settings().get("api_login")))
    #   thread = BitlyExpand(url, string, 15, self.settings.get("api_login"), self.settings.get("api_key"))
    #   threads.append(thread)
    #   thread.start()
    # edit = self.view.begin_edit('bitly')
    # self.handle_threads(edit, threads)
